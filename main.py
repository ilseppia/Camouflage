...
class Button_Camouf(Image):
    ...
    def on_touch_down(self, touch):
        ...
        if Vector(self.center).distance(touch.pos) <= self.height/2:
            ...
            self.source=os.path.join('data','button_red.png') if self.parent.dangerous[self.text]==self.parent.equipment[self.text]+1 else os.path.join('data','button_green.png')
            return True
        return False
...
class GameLayout(RelativeLayout):
    def __init__(self, **kwargs):
        ...
        self.add_widget(Image(source=os.path.join('data','screen_border_front.png')))
